...
class DocToHTMLPostProcessor():
    ...
    def process_html(self, file_path):
        counter = 0
        processed = False
        is_processed_text = '<!-- #DocServer#processed#' + file_path + '# -->'

        if not path.exists('/tmp/docserver'):
            makedirs('/tmp/docserver')

        temp_file = '/tmp/docserver/tmpwrite_' + str(randint(0, 999999))
        source = open(file_path, 'r')
        temporary = open(temp_file, 'w')

        for line in source:
            counter += 1
            if is_processed_text in line:
                processed = True
                break
            elif counter < 5 and not processed:
                processed = False
                break
        ...
        source.seek(0)
        temporary.seek(0)
        temporary.write(is_processed_text + "\n")
